...
const buttons = computed(() => {
  return [
    ...
    { label: t('chat.rename'), type: 'rename', icon: 'i-material-symbols-edit-outline', color: 'gray', class: '', visible: true },
    { label: t('chat.deleteChat'), type: 'delete', icon: 'i-material-symbols-delete-outline', color: 'red', class: '', visible: true },
  ] as const
})

function onClick(type: 'pin' | 'unpin' | 'delete' | 'rename') {
  open.value = false
  switch (type) {
    case 'pin':
