...
import { ANTHROPIC_MODELS, AZURE_OPENAI_GPT_MODELS, GEMINI_EMBEDDING_MODELS, GEMINI_MODELS, GROQ_MODELS, MINIMAX_MODELS, MODEL_FAMILIES, MOONSHOT_MODELS, OPENAI_EMBEDDING_MODELS } from '~/config/index'
...
export const createChatModel = (modelName: string, family: string, event: H3Event): BaseChatModel => {
  ...
  if (customModel && MODEL_FAMILIES.hasOwnProperty(customModel.aiType)) {
    // Handle Azure deployment name
    const params = { 
      ...customModel,
      deploymentName: (customModel as any).deploymentName 
    }
    return initChat(MODEL_FAMILIES[customModel.aiType as keyof typeof MODEL_FAMILIES], modelName, params, true)!
  }
